...
void team_matmul(struct complex ** A, struct complex ** B, struct complex ** C, int a_dim1, int a_dim2, int b_dim2) {
  ...
  for (i = 0; i < a_dim1; i++) {
    for (j = 0; j < b_dim2; j++) {
      ...
      sum.imag = 0.0;
      for (k = 0; k < a_dim2; k++) {
        temp_a = A[i][k].real * B[k][j].real;
        temp_b = A[i][k].imag * B[k][j].imag;
        sum.real += temp_a - temp_b;
        sum.imag += temp_a + temp_b;
      }
      C[i][j] = sum;
    }
  }
}
inline __m128 mul2(float a1, float a2, float b1, float b2){
  __m128 a = _mm_setr_ps(a1,a2,0.0,0.0);
  __m128 b = _mm_setr_ps(b1,b2,0.0,0.0);
